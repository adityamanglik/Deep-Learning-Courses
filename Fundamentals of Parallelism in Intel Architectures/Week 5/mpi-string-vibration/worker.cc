...
#pragma omp parallel for simd
    ...
    if(rank == 0){
      MPI_Send(&d_t[last_segment - 1], 1, MPI_FLOAT, right, sending_tag, MPI_COMM_WORLD);
      MPI_Recv(&d_t[last_segment], 1, MPI_FLOAT, right, sending_tag, MPI_COMM_WORLD, &status);
    }
    else if(rank != world_size - 1){
      //left terminal
      MPI_Send(&d_t[start_segment], 1, MPI_FLOAT, left, sending_tag, MPI_COMM_WORLD);
      MPI_Recv(&d_t[start_segment - 1], 1, MPI_FLOAT, left, sending_tag, MPI_COMM_WORLD, &status);
      //right terminal
      MPI_Send(&d_t[last_segment - 1], 1, MPI_FLOAT, right, sending_tag, MPI_COMM_WORLD);
      MPI_Recv(&d_t[last_segment], 1, MPI_FLOAT, right, sending_tag, MPI_COMM_WORLD, &status);
    }
    else{//rank is world_size - 1
      //left terminal
      MPI_Send(&d_t[start_segment], 1, MPI_FLOAT, left, sending_tag, MPI_COMM_WORLD);
      MPI_Recv(&d_t[start_segment - 1], 1, MPI_FLOAT, left, sending_tag, MPI_COMM_WORLD, &status);
    }
